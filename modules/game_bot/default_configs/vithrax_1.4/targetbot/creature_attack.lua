...
TargetBot.Creature.walk = function(creature, config, targets)
  ...
  if storage.lurePanel.min >= targets then
    storage.TargetBotLure = true
  elseif targets >= storage.lurePanel.max then
  ...
  -- luring
  if TargetBot.canLure() and (config.lure or config.lureCavebot) and not (config.chase and creature:getHealthPercent() < 5) and not isTrapped then
    local monsters = 0
    if storage.TargetBotLure and storage.lurePanel.enabled then
